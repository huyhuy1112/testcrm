						...
						<div class="row" style="padding-top: 2%;">
							<div class="col-sm-12">
								<div class="col-sm-5 calendar-date-time-wrapper">
									...
									{* Time field will be rendered by RECORD_STRUCTURE loop, but we'll enhance it with JS *}
								</div>
								<div class="muted col-sm-1" style="line-height: 67px;left: 20px; padding-right: 7%;">
									{vtranslate('LBL_TO',$MODULE)}
								</div>
								<div class="col-sm-5 calendar-date-time-wrapper" {if $MODULE eq 'Calendar'}style="margin-top: 4%;"{/if}>
									{assign var="FIELD_MODEL" value=$RECORD_STRUCTURE['due_date']}
									{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE)}
									{* Time field will be rendered by RECORD_STRUCTURE loop, but we'll enhance it with JS *}
								</div>
							</div>
							{* All Day Toggle and Duration Display *}
