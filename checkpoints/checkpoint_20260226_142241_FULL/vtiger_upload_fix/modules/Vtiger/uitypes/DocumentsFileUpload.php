	...
	public function getDisplayValue($value, $recordId=false, $recordModel=false) {
		if($recordModel) {
			...
			if ($view === 'Detail' && !empty($value) && $fileStatus) {
				// Trang Detail: hiển thị link download như cũ
				if ($fileLocationType == 'I') {
					...
					if ($fileId) {
						$value = '<a href="index.php?module=Documents&action=DownloadFile&record='.$recordId.'&fileid='.$fileId.'"'.
							' title="'. vtranslate('LBL_DOWNLOAD_FILE', 'Documents').'" >'.textlength_check($value).'</a>';
					} else {
				...
				// List view hoặc trường hợp khác: chỉ hiển thị tên file (chữ thường), không link
				$value = textlength_check($value);
			}
		}
		return $value;
