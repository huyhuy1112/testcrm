...
class Schedule_QuickCreateAjax_View extends Calendar_QuickCreateAjax_View {

	public function requiresPermission(\Vtiger_Request $request) {
		$permissions = parent::requiresPermission($request);
		$record = $request->get('record');
		//Need to check record permission as Schedule view is using QuickCreateAjax to show edit form
	...
	public function process(Vtiger_Request $request) {
		...
		// Schedule uses Calendar and Events modules
		...
		foreach($moduleList as $module) {
			...
			//To enable popup edit support from schedule views
			if($moduleName == $module || $moduleName == 'Schedule') {
				$recordId = $request->get('record','');
				$mode = $request->get('mode');
				if($mode === 'edit' && !empty($recordId)) {
					$recordModel = Vtiger_Record_Model::getInstanceById($recordId,$moduleName == 'Schedule' ? 'Calendar' : $moduleName);
				}
			}
		...
		// For Schedule, use Calendar module fields
		$requestedModuleName = ($moduleName == 'Schedule') ? 'Calendar' : $moduleName;
		$requestedModuleModel = Vtiger_Module_Model::getInstance($requestedModuleName);
		...
		// Add Schedule-specific JS
		$scripts = $this->getHeaderScripts($request);
		if ($moduleName == 'Schedule') {
			$scheduleScript = new Vtiger_JsScript_Model();
			$scheduleScript->set('src', 'layouts/v7/modules/Schedule/resources/ScheduleQuickCreate.js');
			$scheduleScript->set('type', 'text/javascript');
