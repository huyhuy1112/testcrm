...
class Vtiger_MarkNotificationRead_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			if ($mode == 'markAll') {
				$updateSql = "UPDATE vtiger_notifications SET is_read = 1, read_at = NOW() WHERE userid = ? AND is_read = 0";
				$adb->pquery($updateSql, array($userid));
			} else if (!empty($notificationId) && is_numeric($notificationId)) {
				$updateSql = "UPDATE vtiger_notifications SET is_read = 1, read_at = NOW() WHERE id = ? AND userid = ? AND is_read = 0";
				$adb->pquery($updateSql, array($notificationId, $userid));
			} else {
