...
class Vtiger_DocumentsFileUpload_UIType extends Vtiger_Base_UIType {
	...
	public function getDisplayValue($value, $recordId=false, $recordModel=false) {
		if($recordModel) {
			// List: chữ đen, KHÔNG cho tải; Detail: cho tải.
			$view = isset($_REQUEST['view']) ? $_REQUEST['view'] : '';
			$fileLocationType = $recordModel->get('filelocationtype');
			$fileStatus = $recordModel->get('filestatus');

			if ($view === 'Detail' && !empty($value) && $fileStatus) {
				// Trang Detail: hiển thị link download
				if ($fileLocationType == 'I') {
					$db = PearDatabase::getInstance();
					$fileIdRes = $db->pquery('SELECT attachmentsid FROM vtiger_seattachmentsrel WHERE crmid = ?', array($recordId));
					$fileId = $db->query_result($fileIdRes, 0, 'attachmentsid');
					if ($fileId) {
						$value = '<a href="index.php?module=Documents&action=DownloadFile&record='
							.$recordId.'&fileid='.$fileId.'" title="'. vtranslate('LBL_DOWNLOAD_FILE', 'Documents').'">'
					...
					// External URL
					$value = '<a href="'.$value.'" target="_blank" title="'. vtranslate('LBL_DOWNLOAD_FILE', 'Documents').'">'
						.textlength_check($value).'</a>';
				}
			} else {
				// List view hoặc view khác: chỉ hiển thị tên file, không phải link
				$value = textlength_check($value);
			}
		}
		return $value;
