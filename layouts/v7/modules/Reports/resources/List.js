        ...
        app.helper.showConfirmationBox({ message: message }).then(
          function (e) {
            var deleteURL =
              ...
              "&search_params=" +
              searchParams;
            var deleteMessage = app.vtranslate(
              "JS_RECORDS_ARE_GETTING_DELETED"
            );
  ...
  },
  {
    ...
    constructOptionElement: function (info) {
      return (
        ...
        '" data-deleteurl="' +
        info.deleteURL +
        '" class="filterOptionId_' +
        info.folderId +
        ' filterOptionsLabel" id="filterOptionId_' +
        info.folderId +
        '" value="' +
        info.folderId +
        '" data-id="' +
        ...
        app.request
          ...
          .then(function (error, data) {
            ...
            vtUtils.showSelect2ElementView(
              listViewContainer.find("select.select2")
            );
    ...
    deleteFolder: function (event, url) {
      ...
      app.request.get({ url: url }).then(function (error, data) {
        if (data.success) {
          var chosenOption = jQuery(event.currentTarget).closest(
            ".select2-result-selectable"
          );
          var selectOption =
            thisInstance.getSelectOptionFromChosenOption(chosenOption);
          selectOption.remove();
    ...
    registerEditFilterClickEvent: function () {
      ...
      listViewFilterBlock.on("mouseup", "li i.editFilter", function (event) {
        ...
        var liElement = jQuery(event.currentTarget).closest(
          ".select2-result-selectable"
        );
        var currentOptionElement =
          thisInstance.getSelectOptionFromChosenOption(liElement);
    ...
    showQuickPreviewForId: function (recordId) {
      ...
      app.request.get({ data: params }).then(function (err, response) {
        ...
        jQuery(".quickPreviewSummary").trigger(
    ...
    registerEventForPinChartToDashboard: function () {
      ...
      jQuery("html").on("click", ".dashBoardTab", function (e) {
        ...
        var originalDropDownMenu = element
        ...
        app.helper.showConfirmationBox({ message: message }).then(
          function (e) {
            app.request.post({ url: url }).then(function (error, data) {
              if (data.success) {
                app.helper.showSuccessNotification({ message: data.message });
                jQuery('.filterName[data-filter-id="' + folderId + '"]')
                  .closest(".listViewFilter")
                  .remove();
                jQuery('.filterName[data-filter-id="All"]')
                  .closest(".listViewFilter")
                  .click();
              } else {
    ...
    markFolderAsActive: function () {
      ...
      jQuery('.filterName[data-filter-id="' + folder + '"]')
        .closest(".listViewFilter")
        .addClass("active");
    },

    getCurrentCvId: function () {
      return jQuery(".listViewFilter.active")
        .find(".filterName")
        .data("filter-id");
    },
