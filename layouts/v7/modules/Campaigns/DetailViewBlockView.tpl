{* Campaigns: extends Vtiger Detail blocks — hide legacy Product field (keep Products & Services). *}
{*+**********************************************************************************
...
{strip}
	...
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE name=DetailViewBlockViewLoop}
	 ...
	 {/if}
		...
		{if $BLOCK_LABEL_KEY eq 'LBL_CAMPAIGN_PHASES'}{continue}{/if}
					...
					<tbody {if $IS_HIDDEN} class="hide" {/if}>
						...
						<tr>
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								{if $FIELD_MODEL->getName() eq 'product' || $FIELD_MODEL->getName() eq 'product_id'}
									{continue}
								{/if}
...
{if $MODULE_NAME eq 'Campaigns' && $FIELD_MODEL->getName() eq 'description'}
								</tr>
								<tr>
									<td colspan="4" style="padding:12px 16px;border:none;background:rgba(248,250,252,0.95);" class="campaign-under-description-files">
