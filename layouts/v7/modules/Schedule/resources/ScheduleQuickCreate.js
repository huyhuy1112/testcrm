...
(function($){
	...
	var ScheduleQuickCreate = {
		...
		setupAllDayToggle: function() {
			...
			var $allDayCheckbox = $container.find('input[name="allday"], input[data-schedule-allday="1"]');
			...
			$allDayCheckbox.off('change.schedule-allday').on('change.schedule-allday', function() {
		...
		updateDuration: function() {
			...
			var $durationDisplay = $('#schedule-duration-display');
			...
			try {
				...
				if (!isAllDay) {
					...
					if (startTimeStr) startTime = ScheduleQuickCreate.parseTimeToMinutes(startTimeStr);
					if (endTimeStr) endTime = ScheduleQuickCreate.parseTimeToMinutes(endTimeStr);
				}

				// Calculate duration
				var duration = ScheduleQuickCreate.calculateDuration(startDate, endDate, startTime, endTime, isAllDay);
		...
		setupTimeSync: function() {
			...
			$timeStart.on('change blur', function() {
				...
				var endTimeStr = $timeEnd.val();
				var endTime = endTimeStr ? ScheduleQuickCreate.parseTimeToMinutes(endTimeStr) : null;
